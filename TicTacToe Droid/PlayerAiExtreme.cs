...
namespace TicTacToe_Droid
{
    public class PlayerAiExtreme : IPlayer
    {
        ...
        {
            //stop recursive call
            if (GameFlow.IsGameFinished(board))
            {
                finalNodes.Add(node);
                return null;
            }
    ...
    internal class Tree4Ai
    {
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < Board.Length; i++)
            {
                sb.AppendFormat("{0}", GameFlow.BoardValue(Board, i));
                sb.Append((i + 1) % 3 == 0 ? " $ " : ",");
            }
        ...
        public void EvaluateEndNodes()
        {
            ...
            if (GameFlow.DoesPlayerWins(Board, MoveSymbol))
                MinMaxValue = IsMax ? endConstant : -endConstant;
            else if (GameFlow.DoesPlayerWins(Board, !MoveSymbol))
                MinMaxValue = IsMax ? -endConstant : endConstant;
            else
                MinMaxValue = 0;
